...
if st.sidebar.button("🚀 執行全台股盤後掃描"):
    with st.spinner("正在掃描全台股上市+上櫃資料，請稍候..."):
        df_results = run_daily_scan()
...
if not df_results.empty:
    ...
    # 2. 盤中即時監控 (連結 monitor.py)
    st.subheader("⚡ 盤中即時風險監控")
    col1, col2 = st.columns([2, 1])
    with col1:
        st.caption("即時呼叫 `monitor.py` 獲取行情，比對 🛑 ATR 停損 與 🎯 20MA 停利。")
    with col2:
        btn_monitor = st.button("🔄 刷新盤中即時股價", type="primary")
    ...
    if btn_monitor or st.session_state.get('run_monitor_flag', False):
        ...
        def update_ui_progress(current, total, code, name):
            status_text.text(f"【monitor.py】盤中連線中... ({current}/{total}) {code} {name}")
            progress_bar.progress(current / total)

        df_monitor = fetch_realtime_status(filtered_df, progress_callback=update_ui_progress)
    ...
    # 3. 三竹股市快速貼上匯入
    st.subheader("📱 三竹股市 / 券商 App 一鍵貼上匯入")
    sanjuk_code_str = export_sanjuk_format(filtered_df)
    st.info("💡 **手機使用教學**：點擊下圖代碼框右上角複製 -> 開啟三竹股市或券商 App -> 在搜尋列貼上即可一鍵匯入自選清單。")
